...
<div class="wrap">
  ...
  <div class="empty" id="empty">
    ...
    <p>Ask any question. Provide your expected answer. Synapse uses Gemini + web search and scores how close the AI gets.</p>
    <div class="pills">
...
async function doSearch(){
  ...
  try{
    ...
    if(d.ai_answer){
      ans=d.ai_answer;
      document.getElementById('ac').innerHTML=`<div class="badge">🤖 Google Gemini &nbsp;·&nbsp; 🌐 DuckDuckGo</div><div class="ans">${esc(d.ai_answer)}</div>`;
...
async function doScore(){
  ...
  if(!ea||!ans)return;
  sw('sc');
  document.getElementById('sph').innerHTML='<div style="padding:20px;font-size:12px;color:var(--muted)">⏳ AI is evaluating answers...</div>';
  document.getElementById('sph').style.display='block';
  document.getElementById('sd').classList.remove('on');
  try{
    const r=await fetch('/score',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({query:qry,ai_answer:ans,expected_answer:ea})});
    const d=await r.json();
    renderScore(d);
  }catch(e){
